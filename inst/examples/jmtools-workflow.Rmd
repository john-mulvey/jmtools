...
6. Cell type enrichment analysis
...
Generate the QC boilerplate. This example has no precursor-level data, so
`precursor_data_name = NULL` omits that plot:

```{r qc-boilerplate, eval=FALSE}
...
Run the QC checks. The `abundance` matrix is already log2-transformed in this
example; the CV section back-transforms to the linear scale internally.

```{r qc-id-counts, message=FALSE, warning=FALSE}
library(ggplot2)
library(dplyr)

sample_order <- sample_meta$sample_id[order(sample_meta$group)]

# Proteins quantified per sample
protein_counts <- data.frame(
  sample_id  = colnames(abundance),
  n_proteins = colSums(!is.na(abundance))
)
protein_counts <- merge(protein_counts, sample_meta, by = "sample_id")

ggplot(protein_counts,
       aes(x = factor(sample_id, levels = sample_order),
           y = n_proteins, fill = group)) +
  geom_col() +
...
```{r qc-missing-bar, message=FALSE, warning=FALSE}
n_features <- nrow(abundance)
missing_df <- data.frame(
  sample_id   = colnames(abundance),
  pct_missing = colSums(is.na(abundance)) / n_features * 100
)
missing_df <- merge(missing_df, sample_meta, by = "sample_id")
missing_df$sample_id <- factor(missing_df$sample_id, levels = sample_order)

ggplot(missing_df, aes(x = sample_id, y = pct_missing, fill = group)) +
  geom_col() +
...
```{r qc-missing-heatmap, message=FALSE, warning=FALSE, fig.height=6}
library(pheatmap)

is_missing   <- is.na(abundance)
has_any_miss <- rowSums(is_missing) > 0

if (sum(has_any_miss) > 0) {
  ...
  message("No missing values in this dataset - heatmap omitted.")
}
```

```{r qc-completeness, message=FALSE, warning=FALSE}
...
```{r qc-correlation, message=FALSE, warning=FALSE}
library(ggcorrplot)

cor_mat <- cor(abundance, use = "pairwise.complete.obs", method = "pearson")
...
```{r qc-cv, message=FALSE, warning=FALSE}
group_levels <- levels(as.factor(sample_meta$group))

cv_list <- lapply(group_levels, function(grp) {
  cols <- sample_meta$sample_id[sample_meta$group == grp]
  mat  <- 2^abundance[, cols, drop = FALSE]
  cv   <- apply(mat, 1, function(x) {
    x <- x[!is.na(x)]
    if (length(x) < 2) NA_real_ else sd(x) / mean(x) * 100
  })
  data.frame(cv = cv, group = grp, stringsAsFactors = FALSE)
})
cv_df <- do.call(rbind, cv_list)
cv_df <- cv_df[!is.na(cv_df$cv), ]
cv_df$group <- factor(cv_df$group, levels = group_levels)

ggplot(cv_df, aes(x = cv, colour = group, fill = group)) +
...
Quantify collinearity between clinical variables and group membership using
point-biserial correlation. Two-level factors (e.g. Sex) are automatically
encoded as 0/1. This informs whether covariates are likely to confound a
limma model.

```{r confounding-analysis, message=FALSE, warning=FALSE}
confound_results <- test_variable_group_associations(
  metadata       = sample_meta,
  vars_to_test   = c("Sex", "Age", "BMI", "Ejection fraction (%)"),
  group_var      = "group",
...
```{r confounding-plot, message=FALSE, warning=FALSE}
plot_variable_group_associations(confound_results)
```
...
Cell type enrichment asks whether proteins that are differentially abundant
between groups tend to be markers of a particular cell type, as defined by
snRNAseq data. Here we use the Human Heart Cell Atlas v2 Level 1 marker genes
and test for enrichment using GSEA against the limma t-statistic ranking.

> **Note:** The marker gene file bundled with this package
> (`example_marker_genes.csv`) is a **trimmed example dataset** - the top 20
> markers per cell type by AUC from the Heart Cell Atlas v2, included solely
> to make this vignette runnable. It is not suitable for real analysis. See
> `?filter_marker_genes` for the path to the full dataset on the group drive,
> and `inst/extdata/generate_marker_genes_example.R` for how this file was
> generated.

### Load and Filter Marker Genes

```{r celltype-load}
marker_genes_raw <- read.csv(
  system.file("extdata", "example_marker_genes.csv", package = "jmtools"),
...
### Effect Size Distribution by Cell Type

Visualise the distribution of log2 fold changes across each cell type's marker
proteins. Cell types whose markers are systematically up- or down-regulated
will stand out from the others:

```{r celltype-effect-sizes, fig.height=5}
plot_celltype_effect_sizes(
  markers_filtered,
...
Confirm that the top markers used as gene sets show the expected cell-type-
specific expression patterns in the snRNAseq reference:

```{r celltype-dotplot, fig.height=6, fig.width=10}
top_genes <- tapply(
  markers_filtered$gene,
  markers_filtered$cluster,
  function(g) head(g, 5)
)

plot_celltype_dotplot(marker_genes_raw, genes = unique(unlist(top_genes)))
```

### Cell Type GSEA

Build a named gene set list from the filtered markers (one set per cell type)
and run GSEA directly with `fgsea`, using the limma t-statistic as the ranking:

```{r celltype-gsea}
library(fgsea)

celltype_gene_sets <- split(markers_filtered$gene, markers_filtered$cluster)

ranks_celltype <- limma_results_heart_failure_vs_control |>
  dplyr::filter(!is.na(t), !is.na(gene_symbol)) |>
  dplyr::group_by(gene_symbol) |>
  dplyr::slice_max(abs(t), n = 1, with_ties = FALSE) |>
  dplyr::ungroup() |>
  (\(d) setNames(d$t, d$gene_symbol))() |>
  sort(decreasing = TRUE)

set.seed(42)
gsea_celltype <- fgsea(
  pathways = celltype_gene_sets,
  stats    = ranks_celltype,
  minSize  = 5,
  maxSize  = 500
) |>
  tibble::as_tibble() |>
...
```{r celltype-bars, fig.height=5}
plot_gsea_bars(
  gsea_celltype,
  gene_set_col = "cell_type",
  score_col    = "nes",
  x_label      = "Normalised Enrichment Score (NES)",
  title        = "Cell Type Enrichment (GSEA)"
)
```
